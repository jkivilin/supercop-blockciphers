...
extern void blowfish_enc_blk_16way(struct blowfish_ctx *ctx, uint8_t *dst, const uint8_t *src);
...
#define PTR_ALIGN(ptr, mask) ((void *)((((long)(ptr)) + (mask)) & ~((long)(mask))))
	...
	while (likely(inlen >= BLOCKSIZE * 16)) {
		...
		blowfish_enc_blk_16way(ctx, out, (uint8_t *)ivs);
	...
	if (unlikely(inlen > 0)) {
		...
		blowfish_enc_blk_16way(ctx, (uint8_t *)ivs, (uint8_t *)ivs);
