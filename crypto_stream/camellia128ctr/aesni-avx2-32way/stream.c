/*
 * SuperCop glue code for Camellia-AVX2&AESNI-32way
 *
...
extern void camellia_ctr_32way(struct camellia_ctx *ctx, void *out, const void *in, uint128_t *iv, unsigned long chunks);
...
#define PTR_ALIGN(ptr, mask) ((void *)((((long)(ptr)) + (mask)) & ~((long)(mask))))
	...
	if (likely(inlen >= PARALLEL_BLOCKS * BLOCKSIZE)) {
		...
		camellia_ctr_32way(ctx, out, in, &iv, chunks);
	...
	if (unlikely(inlen > 0)) {
		...
		camellia_ctr_32way(ctx, buf, NULL, &iv, 1);

		if (in) {
			for (i = 0; inlen >= BLOCKSIZE * 1; i += 1) {
